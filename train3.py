...
from model3 import SimpleLossCompute, LabelSmoothing, NoamOpt
from model3 import make_model
...
log_dir = os.path.join('tensorboard', 'cdt_train')
...
log_dir = os.path.join('tensorboard', 'cdt_test')
if not os.path.exists(log_dir):
    os.mkdir(log_dir)
    print(f"{log_dir}创建成功！")
...
def train(data, model, criterion, optimizer):
    ...
    for epoch in range(EPOCHS): # EPOCES在代码开头指定
        ...
        train_writer.add_scalar('cdt_Train_loss', train_loss, epoch)
        model.eval()
        ...
        if exit_num == 10:
            ...
            break
        print()
        test_writer.add_scalar('cdt_Dev_loss', dev_loss, epoch)
