...
class MultiHeadAttention2(nn.Module):
    ...
    def forward(self,
        ...
        q_1 = q_0[:,:, 0:256].reshape(n, q_len, 2, 128).transpose(1, 2)
        k_1 = k_0[:,:, 0:256].reshape(n, k_len, 2, 128).transpose(1, 2)
        v_1 = v_0[:,:, 0:256].reshape(n, k_len, 2, 128).transpose(1, 2)

        q_2 = q_0[:,:, 256:384].reshape(n, q_len, 2, 64).transpose(1, 2)
        k_2 = k_0[:,:, 256:384].reshape(n, k_len, 2, 64).transpose(1, 2)
        v_2 = v_0[:,:, 256:384].reshape(n, k_len, 2, 64).transpose(1, 2)

        q_3 = q_0[:,:, 384:].reshape(n, q_len, 4, 32).transpose(1, 2)
        k_3 = k_0[:,:, 384:].reshape(n, k_len, 4, 32).transpose(1, 2)
        v_3 = v_0[:,:, 384:].reshape(n, k_len, 4, 32).transpose(1, 2)
        ...
        concat_res = torch.cat([concat_res1,concat_res2,concat_res3], -1)
...
def make_model(src_vocab, tgt_vocab, N=LAYERS, d_model=D_MODEL, d_ff=D_FF, h=H_NUM, dropout=DROPOUT):
    c = copy.deepcopy
    # 实例化Attention对象
    attn = MultiHeadAttention2(h, d_model).to(DEVICE)
    # 实例化FeedForward对象
