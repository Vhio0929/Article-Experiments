...
from model4 import SimpleLossCompute, LabelSmoothing, NoamOpt
from model4 import make_model
...
log_dir = os.path.join('tensorboard', 'zsy+k_train')
...
log_dir = os.path.join('tensorboard', 'zsy+k_test')
...
def train(data, model, criterion, optimizer):
    ...
    for epoch in range(EPOCHS): # EPOCES在代码开头指定
        ...
        train_writer.add_scalar('zsy_Train_loss', train_loss, epoch)
        model.eval()
        ...
        if exit_num == 5:
            ...
            break
        print()
        test_writer.add_scalar('zsy_Dev_loss', dev_loss, epoch)
