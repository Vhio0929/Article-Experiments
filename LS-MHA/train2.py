...
from model2 import SimpleLossCompute, LabelSmoothing, NoamOpt
...
def train(data, model, criterion, optimizer):
    ...
    for epoch in range(EPOCHS): # EPOCES在代码开头指定
        ...
        if dev_loss < best_dev_loss:
            torch.save(model.state_dict(), 'save/ori16.pt')
            best_dev_loss = dev_loss
            print('****** Save model done... ******')
        if epoch == 100:
            torch.save(model.state_dict(), 'save/ori16_100.pt')
        if exit_num == 10:
            print("====超过10轮未增长，保存退出====")
            break
        print()
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join('5_1_tensorboard4', 'ori_train')
    ...
    # 将 test accuracy 保存到 "5_1_tensorboard4/test" 文件夹
    log_dir = os.path.join('5_1_tensorboard4', 'ori_dev')
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
        print(f"{log_dir}创建成功！")
