...
class PrepareData:
    def __init__(self, train_file, dev_file, sort=False):
        # 读取数据 并分词
        ...
        self.train_en, self.train_cn = self.wordToID(self.train_en, self.train_cn, self.en_word_dict, self.cn_word_dict, sort)
        self.dev_en, self.dev_cn = self.wordToID(self.dev_en, self.dev_cn, self.en_word_dict, self.cn_word_dict, sort)
    ...
    def load_data(self, path):
        ...
        word_tokenize_de = get_tokenizer('spacy', language='de_core_news_sm')
        word_tokenize_en = get_tokenizer('spacy', language='en_core_web_sm')
        """
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                #en.append(["BOS"] + word_tokenize(line[0]) + ["EOS"])
                #cn.append(["BOS"] + word_tokenize(line[1].lower()) + ["EOS"])
                en.append(["BOS"] + word_tokenize_de(line[0]) + ["EOS"])
                cn.append(["BOS"] + word_tokenize_en(line[1].lower()) + ["EOS"])
...
class Batch:
    ...
    def __init__(self, src, trg=None, pad=0):
        ...
        # print("self.src.shape:", self.src_mask.shape)

        assert self.src_mask.shape[0] == src.shape[0]
        assert self.src_mask.shape[2] == src.shape[1]
